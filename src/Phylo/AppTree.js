...
export default function AppTree() {
  ...
  const handleFileRead = (e) => {
	...
	const RenderComponent = React.forwardRef((props, ref2) =>(
    
      layout==="linear" && selectedFile  ?(<Tree
        data={selectedFile}
        clickName={d => {
          console.log(d)
        }}
    ...
    return (
      <div style={{display:'flex',height:500,flexDirection:"column",width:"100%",marginTop:"2em"}}>
        <div>
        ...
        />
        <label for="branchlengthnumber">Show branch length Number</label>
        <input
          type="checkbox"
          id="branchlengthnumber"
          checked={value}
          onChange={() => setShowBranchLengthNumber(!showBranchLengthNumber)}
        />
      <div>
      <input
      type='file'
    ...
    />
          ...
          <input type="text" placeholder="search" onChange={e => setfName(e.target.value)} />
          <button onClick={()=>search(fname)}>search</button>
          <button onClick={()=>hiddenLeaves()}>hiddenLeaves</button>
          <button onClick={()=>setHorizontal(horizontal+0.1)}>horizontal</button>
          <button onClick={()=>setSwap(swap*-1)}>swap</button>


          <button onClick={()=>changeLayout()}>{layout==="linear" ? "linear" : "circular"}</button>
          {
            layout==="circular" ? (<button onClick={()=>setCircularNumber(circularNumber+0.5)}>+ cicular</button>) : null
          }
        </div>
